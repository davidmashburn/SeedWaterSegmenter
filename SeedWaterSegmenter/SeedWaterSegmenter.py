...
try:
    ...
    def array2cv(a):
        ...
        try:
            ...
            nChannels = 1
        cv_im = cv.CreateImageHeader(
            (a.shape[1], a.shape[0]), dtype2depth[str(a.dtype)], nChannels
        )
...
(DONT_PANIC, DEFAULT_SEED_SIZE, USE_DEBUG_PRINT, STEAL_B_KEY, CAT_SPACED_ARGS) = (
    _USER_OPTIONS_TABLE[username]
    if username in _USER_OPTIONS_TABLE
    else _USER_OPTIONS_TABLE[None]
)
...
txtHeader = [
    "# SeedWater notes file version 1",
...
def CreateOutlines(arr, walgorithm="PyMorph"):
    if walgorithm in [
        "OpenCV"
    ]:  # This is now deprecated and will not produce correct results on saving!
        return arr == -1
...
def CreateThickOutlines(arr, thickness=1):
    ...
    if thickness > 1:
        newArr = scipy.ndimage.morphology.binary_dilation(
            newArr, iterations=thickness - 1
        )
    return newArr
...
def GetReportPixel(sfORwd):  # create and return the report_pixel function...
    def report_pixel(x, y):
        if isinstance(sfORwd, SegmenterFrame):
            ...
            raise (
                TypeError("sfORwd must be a WatershedData or SegmenterFrame Object!")
            )
...
def meanN(l):
    ...
    return sumN(l) * 1.0 / len(l)


def meanN_skip(l):
    return sumN(l) * 1.0 / sum([(0 if i is None else 1) for i in l])


def CreateTimeAxis(timeIntervals, gapIntervals, numberFramesPerSeries, numberOfFrames):
    if not (
        len(timeIntervals) == (len(gapIntervals) + 1) == len(numberFramesPerSeries)
    ):
        print(
            "intervalList and numberWintervalList must have the same length to be valid!!!"
        )
    ...
    for i in range(len(timeIntervals)):
        ran = np.arange(
            0, timeIntervals[i] * numberFramesPerSeries[i], timeIntervals[i]
        )
        if i == 0:
            add = 0
        else:
...
def GetBinnedValueForExcel(value, name, bins, timeAxis, woundVals):
    ...
    valueBinned = [
        [["Wound " + name, None, "Time"]]
        + [
            [p, None, timeAxis[i]]
            for i, p in enumerate(map(sumN, value[:, woundVals - np.r_[2]]))
        ]
    ]
    ...
    firstSheet = [
        [timeAxis[i], p] + [m[b][i] for b in range(len(bins))]
        for i, p in enumerate(map(sumN, value[:, woundVals - np.r_[2]]))
    ]
    valueBinned.insert(
        0,
        [["time", "wound"] + ["ring" + str(i + 1) for i in range(len(bins))]]
        + firstSheet,
    )
    return valueBinned
...
def MergeWoundVals(neigh, wvl):
    ...
    for i in range(len(neigh)):
        if neigh[i] != None:
            neigh[i] = sorted(
                list(set([(firstWV if (j in wvl) else j) for j in neigh[i]]))
            )
...
class WatershedDataCore(object):
    def __init__(self, arrayIn, previousSeeds=None):
        ...
        #    self.origData *= 2**8
            ...
            self.origData = GTL.DivideConvertType(
                arrayIn,
                bits=16,
                maxVal=2 ** 15 - 1,
                zeroMode="stretch",
        ...
        self.sparseList = [
            None
        ] * self.length  # will be a list of sparse matrices (lil format)
        ...
        self.mapPlotRandomFloatArray = (
            np.array(self.mapPlotRandomArray, dtype=np.float) / 255.0
        )
        self.mapPlotCmap = matplotlib.colors.ListedColormap(
            self.mapPlotRandomFloatArray.T
        )
        self.valList = None
    ...
    def Undo(self):
        ...
        if None not in [self.oldSparseList, self.oldSeedSelections]:
            ...
            self.sparseList[self.index], self.oldSparseList = (
                self.oldSparseList,
                self.sparseList[self.index],
            )  # SWAP
            self.seedSelections[self.index], self.oldSeedSelections = (
                self.oldSeedSelections,
                self.seedSelections[self.index],
            )  # SWAP
            self.point_mode, self.old_point_mode = (
                self.old_point_mode,
                self.point_mode,
            )  # SWAP
            self.previousDrawPoint = None
        ...
        for line in fid:
            ...
            if len(line) > lA + lB:
                if (
                    line[:lA] == txtsepA
                    and line[-lB:] == txtsepB
                    and line[lA:-lB].isdigit()
                ):
                    fn = int(line[lA:-lB])
                    if fn > fnLast:
                        self.notes += [""] * (
                            fn - fnLast
                        )  # Add entries for each frame to notes
                        fnLast = fn
                        newFrame = True
                    else:
    ...
    def Save(self, d, saveOutlines=True):  # Ignore Undo
        ...
        GTL.SaveFileSequence(
            self.watershed,
            basename=segmentsBase,
            im_format="tif",
            sparseSave=self.framesVisited,
        )

        if saveOutlines:
            getOutlines = lambda arr: CreateOutlines(
                arr, walgorithm="PyMorph"
            )  # This will break the deprecated OpenCV watershed
            GTL.SaveFileSequence(
                self.watershed,
                basename=outlinesBase,
                im_format="tif",
                sparseSave=self.framesVisited,
        ...
        seedList = [
            (None if i is None else np.array([i.row, i.col]).T.tolist())
            for i in cooList
        ]
        seedVals = [
            (None if i is None else i.data.astype(np.int).tolist()) for i in cooList
        ]

        WriteSeedPointsFile(
            seedPointsFile, seedList, seedVals, self.walgorithm, self.woundCenters
        )
        ...
        if len(Seeds.seedList) == self.length:
            for i in range(self.length):
                if None in [Seeds.seedList[i], Seeds.seedVals[i]]:
                    ...
                    self.sparseList[i] = scipy.sparse.lil_matrix(
            ...
            for i in range(self.length):
                if self.sparseList[i] is not None:
                    self.seedSelections[i] = scipy.sparse.lil_matrix(
                        self.shape[1:], dtype=np.bool
                    )
    ...
    def UpdateSeeds(self, force=False):
        ...
        if force == True:
            self.SetUndoPoint()
            maxMap = MaxMinFinder(
                gaussian_filter(self.origData[self.index], self.gaussSigma)
            )  # Max pts of gauss filtered image (single pixels with values of 1)
            row, col = np.where(maxMap)
            self.seedArray[:] = 0
            for i in range(len(row)):
                self.seedArray[row[i], col[i]] = (
                    i + 2
                )  # 0 and 1 are reserved for unfilled region and background value respectively
            self.seedArray = scipy.ndimage.morphology.grey_dilation(
                self.seedArray, footprint=ImageCircle(DEFAULT_SEED_SIZE)
            )
            self.sparseList[self.index] = scipy.sparse.lil_matrix(
                self.seedArray, dtype=np.uint16
            )
            self.seedSelections[self.index] = scipy.sparse.lil_matrix(
                self.shape[1:], dtype=np.bool
            )
        else:
        ...
        if self.background:
            val = (
                2
            )  # (2 if self.walgorithm=='cv' else 1) # I could do this, but what's the need?
    ...
    def Watershed(self, algorithm="PyMorph"):  # Ignore Undo
        ...
        if self.walgorithm[self.index] == "OpenCV":
            self.watershed[self.index] = cvWater(
                GTL.DivideConvertType(self.filterData[self.index], 8), self.seedArray
            )
            ...
            self.watershed[self.index] = mahotas.cwatershed(
                self.filterData[self.index], self.seedArray
            )
        ...
        self.woutline = CreateOutlines(
            self.watershed[self.index], walgorithm=self.walgorithm[self.index]
        )
    ...
    def MakeSeedsFromPrevious(self):
        if self.index > 0:
            ...
            for cm in cmList:
                ...
                self.seedArray[cm[0], cm[1]] = self.watershed[self.index - 1][
                    cm[0], cm[1]
                ]
            self.seedArray = scipy.ndimage.morphology.grey_dilation(
                self.seedArray, footprint=ImageCircle(DEFAULT_SEED_SIZE)
    ...
    def CopySeedsFromPrevious(self):
        if self.index > 0:
            ...
            self.seedSelections[self.index] = deepcopy(
                self.seedSelections[self.index - 1]
            )
            self.UpdateSeeds()
            self.Watershed()
    ...
    def NewSeed(self, pointIn, val=None):
        ...
        newPoints = GetPointsAtRadius(
            self.seedArray.shape, pointIn[0], pointIn[1], self.pointSize
        )
            ...
            if val != newVal:
                for i in range(numFrames):
                    ...
                    if self.oldSparseList is not None:
                        ...
                        self.oldSparseList = scipy.sparse.lil_matrix(
                            oldSeedArray, dtype=np.uint16
                        )
    ...
    def AutoCenterWound(self, woundVals):
        ...
        for self.index in range(self.length):  # For each frame
            if self.sparseList[self.index] is not None:
                newWVal = (
                    1e6
                )  # I don't think anyone will ever create a million cells...
                wi = np.array(self.watershed[self.index], dtype=np.int)
                for v in woundVals:
                    wi[np.where(wi == v)] = newWVal
                test = wi == newWVal
                if np.any(test):
                    self.woundCenters[self.index] = center_of_mass(
    ...
    def GetManualInputs(self, d):
        ...
        if not os.path.exists(ManualInputs):
            ...
            te.SetInitialSize(
                size=(-1, 250)
            )  # set initial size of the window to be 300
            te.ShowModal()
        ...
        try:
            ...
            print(
                "All the variables (woundVals,timeIntervals,gapIntervals,numberFramesPerSeries) must be defined!"
            )
            return
        return MI
...
def _data_splitter_for_excel(data, adjLength):
    ...
    s = "\n".join(
        [
            "There are too many frames ({}) for one excel sheet (max 256)!".format(
                adjLength
            ),
    ...
    if (
        len(data[0]) > 256
    ):  # This will break the excel write, so make break into pieces...
        numBreakups = (len(data[0]) - 1) / 256 + 1
        indexList = [256 * i for i in range(numBreakups)] + [len(data[0])]
        dataSets = [
            [d[indexList[f] : indexList[f + 1]] for d in data]  # each parameter
            for f in range(numBreakups)
        ]  # each file
    else:
        dataSets = [data]
...
class WatershedDataCoreWithStats(WatershedDataCore):
    def __init__(self, *args, **kwds):
        ...
        excel_stats_table = (
            (self.valuesByFrame, "CellValuesByFrame"),
            ...
            for j in range(sh[1]):
                if (
                    len(
                        set(
                            [
                                self.watershed[index][i][j],
                                self.watershed[index][i + 1][j],
                                self.watershed[index][i][j + 1],
                                self.watershed[index][i + 1][j + 1],
                            ]
                        )
                    )
                    > 2
        ...
        for t in range(len(tj)):
            idsByTJ.append(
                list(
                    set(
                        [
                            self.watershed[index][tj[t][0] + x][tj[t][1] + y]
    ...
    def GetPolygonsAndPerimeters(self):
        ...
        for index in range(lastActiveFrame + 1):
            tj, idList, idsByTJ, tjsByID = self.GetTripleJunctionPointsAndVals(
                index
            )  # Runs an Update
    ...
    def CalculatePerimeter(self, boundsList=None):
        ...
        for i, v in enumerate(self.valList):
            perimeterList.append(
                ImageContour.GetIJPerimeter(
                    self.watershed[self.index], v, boundsList[i]
                )
            )
        ...
        for i, v in enumerate(self.valList):
            ...
            ellipseList.append(
                EllipseFitter.EllipseFitter(
                    self.watershed[self.index][xm : xM + 1, ym : yM + 1] == v
                )
            )
        return ellipseList
    ...
    def CalculateWoundDistance(self, cmList=None):
        ...
        for [x, y] in cmList:
            ...
            theta = np.arctan2(1.0 * (x - woundX), 1.0 * (y - woundY))
            rList.append(r)
    ...
    def CalculateNeighbors(self, boundsList=None):
        ...
        for i, v in enumerate(self.valList):
            ...
            if (
                xm == 0
                or ym == 0
                or xM == self.watershed.shape[1] - 1
                or yM == self.watershed.shape[2] - 1
            ):
    ...
    def CalculateEdgeBrightnessValues(self, thickness=1, saveDir=None):
        ...
        if saveDir:
            outlDir = os.path.join(
                saveDir, "ThickOutlines" + (str(thickness) if thickness > 1 else "")
            )
            if not os.path.exists(outlDir):
                os.mkdir(outlDir)
            GTL.SaveSingle(
                255 * mask, os.path.join(outlDir, "Mask%i" % self.index), tiffBits=8
            )

        edgeBrightnessValues = sorted(
            self.origData[self.index][np.where(mask)].tolist()
        )
    ...
    def CollectAllStats(self, saveDirForOutlineMasks=None):
        ...
        for index in range(adjLength):
            ...
            self.edgeBrightnessValues.append(
                self.CalculateEdgeBrightnessValues(saveDir=saveDirForOutlineMasks)
            )
        ...
        for v in range(
            2, maxSeedVal + 1
        ):  # This can get ugly if there is too much deleting and what-not...
            ...
            for i in range(len(self.valuesByFrame)):
                if not v in self.valuesByFrame[i]:
                    for m in self.all_stats[
                        1:
                    ]:  # skip the first stat, self.valuesByFrame... this made this measure totally worthless and confusing
                        m[i].insert(v - 2, None)
    ...
    def SaveAllStats(self, directory=None):
        ...
        if not all(self.all_stats):
            ...
            s = os.linesep.join(
                [
                    "Skip saving stats because some stats are missing!",
    ...
    def SaveTripleJunctionsWithCellIDs(self, directory=None):
        ...
        for i in range(adjLength):
            ...
            for t in range(len(tj[0])):
                cellIDs = list(
                    set(
                        [
                            self.watershed[i][tj[0][t] + x][tj[1][t] + y]
                            for x, y in [[0, 0], [1, 0], [0, 1], [1, 1]]
                        ]
                    )
                )
        ...
        ExcelHelper.excelWrite(
            bigData,
            [str(i + 1) for i in range(adjLength)],
            os.path.join(directory, "TripleJunctionsWithCellIDs.xls"),
        )

    # NOT TESTED
    def GetNeighborPairsByFrame(
        self, woundVals
    ):  # Merges wound vals... make sure you run LoadStats or CollectAllStats first!
            ...
            for i in range(len(self.neighbors[frame])):
                if self.neighbors[frame][i] is not None:
                    if len(self.neighbors[frame][i]) > 0:
                        neighborPairs.update(
                            [
                                tuple(sorted([i + 2, j]))
                                for j in self.neighbors[frame][i]
                            ]
                        )
    ...
    # NOT TESTED
    def GetContourValuesLengthsAndSubContoursByFrame(
        self, allValsByFrame, woundValsSorted
    ):
        ######### NOT DONE!!!!!!! ####################
        ...
        for frame in range(self.length):
            ...
            for v in allValsByFrame[frame]:
                ...
                perimeterVals, perimeterList, subContours = ImageContour.GetPerimeterByNeighborVal(
                    wi, v, boundingRect=boundingRect, getSubContours=True
                )
                subContoursAdj = [
                    (np.array(sc) + [boundingRect[0][0], boundingRect[1][0]]).tolist()
                    for sc in subContours
                ]  # Will need to - 0.5 to line up on an overlay
                if len(perimeterList) > 0:
                    cVLS += [
                        [
                            sorted([v, perimeterVals[i]]),
                            perimeterList[i],
                            subContoursAdj[i],
                        ]
            ...
            for i in range(len(cVLS) - 1, 0, -1):
                if cVLS[i - 1][0] == cVLS[i][0]:  # if 2 subcoutours are the same,
                    cVLS[i - 1][1] = min(
                        cVLS[i - 1][1], cVLS[i][1]
                    )  # keep only the one with the minimum length computation
                    del cVLS[i]
    ...
    def SaveSubContours(self, directory=None):
        ...
        cVLSByFrame = self.GetContourValuesLengthsAndSubContoursByFrame(
            allValsByFrame, woundValsSorted
        )
        ...
        for stat, name in zip(self.csv_stats, self.csv_stat_names):
            ...
            if os.path.exists(nameCsv):
                ...
                int_fields = [
                    ...
                    "Ymin",
                    "Ymax",
                    "EdgeBrightnessValues",
                ]
                conv = IorN if name in int_fields else ForN
                stat[:] = [
                    map(conv, line.replace(" ", "").split(",")) for line in lines
                ]
            else:
    ...
    def CreateBinsWDistance(self, frameToCompare, woundVals, binSize=30, initR=0):
        if not self.woundDistance:
            print(
                "Error! Need have collected stats for the data before running CreateBinsWDistance!"
            )
            return
    ...
    def CreateBinsWNeighbors(self, frameToCompare, woundVals):
        if not self.neighbors:
            print(
                "Error! Need have collected stats for the data before running CreateBinsWNeighbors!"
            )
            return
                ...
                if nei[w] is not None:
                    for v in nei[w]:
                        if v > 1:  # skip background
                            bins[-1].append(v - 2)
            bins[-1] = list(
                set(bins[-1]).difference(*([wm2] + map(set, bins[:-1])))
            )  # remove duplicates...
            if bins[-1] == []:
                break
        while bins[-1] == []:
    ...
    def GetWoundNeighborContourLengths(
        self, index, woundVals, printPerimeterCheck=False
    ):
        ...
        for i in range(len(WNv)):
            ...
            if WNv != NWv:  # WHAT DOES THIS REALLY DO???
                print(
                    "Error!!! Wound-Neighbor contours do not match Neighbor-Wound contours!"
                )
                break
            ...
            if self.sparseList[index] is not None:
                if self.sparseList[index].nnz != 0:
                    for i in [WNv, WNl, NNv, NNl, NOv, NOl]:
                        i.append([])
                    WNv[-1], WNl[-1], NNv[-1], NNl[-1], NOv[-1], NOl[
                        -1
    ...
    def GetBinnedContours(self, woundVals, WNv, WNl, NNl, NOl, usePrint=True):
        """Return contours for Wound Perimeter (3 versions)""" + """                    Length of Tangential Boundaries from Wound""" + """                    Total Perimeter Around First Ring Of Cells Around Wound (2 versions)"""
        ...
        for frame in range(self.length):
            ...
            # Total P of all NNs
            totalWN_Per = sumN(
                [self.perimeter[frame][v - 2] for v in WNv[frame]]
            )  # I think for this we really need the local vals for each frame...
    ...
    def PerimeterTests(
        self, frame, testInd, woundVals, wcList, swcList, useSorted=False
    ):
        ...
        if not self.perimeter:
            print(
                "Error! Must collect stats for the data before running PerimeterTests!"
            )
            return
        ...
        if useSorted:
            print(
                sumN(np.array(WNl_byVal)[:, frame])
                + 2 * sumN(np.array(NNl_byVal)[:, frame])
                + sumN(np.array(NOl_byVal)[:, frame])
            )
        else:
            print(sumN(WNl[frame]) + 2 * sumN(NNl[frame]) + sumN(NOl[frame]))
        print("Num of WN vals:")
        if useSorted:
            print(len(WNsortedVals))
        else:
            print(len(WNv[frame]))

        if useSorted:
            testVal = WNsortedVals[testInd]
        else:
            testVal = WNv[frame][testInd]

        print("Total P for", testVal, "w/GP")
        print(self.perimeter[frame][testVal - 2])
        vals, pers = ImageContour.GetPerimeterByNeighborVal(
            self.watershed[frame], testVal
        )
        ...
        if useSorted:
            s1 = WNl_byVal[testInd][frame]
        else:
            s1 = WNl[frame][testInd]
    ...
    def RunCalculations2(self, d):
        ...
        for i in range(self.length):
            if self.sparseList[i] is None:
                ...
                if not hasWound:
                    ...
                    print(
                        "Error! You must have a wound region in all frames to use RunCalculations2!!!"
                    )
                    print("Option 1: If you messed up the wound values, correct them.")
                    print(
                        "Option 2: Go through each initialized frame and add at least a small bit of wound where it might be."
                    )
                    print(
                        "          Then re-run RunCalculations and then try RunCalculations2."
                    )
                    return
        ...
        if useNei:
            ...
            bins = self.CreateBinsWDistance(
                BinningFrame, woundVals, binSize=binSize, initR=initR
            )
        # etc...
        ...
        # Hitting Error Here:
        WNv, WNl, NNv, NNl, NOv, NOl, WNsortedVals, WNl_byVal, NNsortedVals, NNl_byVal, NOsortedVals, NOl_byVal = self.GetWNOContourLengthsSorted(
            woundVals
        )
        WP, WPc, WPc2, spokes, WNP, WNPc = self.GetBinnedContours(
            woundVals, WNv, WNl, NNl, NOl, usePrint=False
        )

        ExcelHelper.excelWrite(
            [
                [
                    ["Time Axis"] + timeAxis[: len(WP)],
                    ["Wound Perimeter"] + WP,
            ...
            ["Contour Lengths"],
            os.path.join(d, "ContourLengths.xls"),
            flip=True,
        )

        areaBinned = GetBinnedValueForExcel(
            self.area, "Area", bins, timeAxis, woundVals
        )
        ...
        ExcelHelper.excelWrite(
            areaBinned,
            sheetnames=["Summary", "Wound"]
            + [
                GetPlacementName(i - 1) + " Ring of Cells Around Wound"
                for i in range(2, len(areaBinned))
            ],
            filename=os.path.join(d, "BinnedAreas.xls"),
        )
        # Output a binned file for major and minor, too...
        majorBinned = GetBinnedValueForExcel(
            self.major, "Minor Axis", bins, timeAxis, woundVals
        )
        minorBinned = GetBinnedValueForExcel(
            self.minor, "Major Axis", bins, timeAxis, woundVals
        )
        ExcelHelper.excelWrite(
            majorBinned,
            sheetnames=["Summary", "Wound"]
            ...
            for b in range(len(bins)):
                ...
                for i, wa in enumerate(woundAngles):
                    ...
                    if None in (majorBin[-1], minorBin[-1]):
                        ...
                        ax_ththBin.append(None)
                        ax_rrBin.append(None)
                        ax_ththBin[-1], ax_rrBin[
                            -1
                        ] = EllipseFitter.GetRotatedMajorMinor(
                            majorBin[-1], minorBin[-1], angleBin[-1], wa * 180 / np.pi
                        )
        ...
        ExcelHelper.excelWrite(
            excelOut, sheetNames, os.path.join(d, "PrincipalAxes.xls"), flip=False
        )
...
class WatershedData(WatershedDataCoreWithStats):
    ...
    def __init__(
        self, arrayIn, previousSeeds=None, fig1=None, ax1=None, fig2=None, ax2=None
    ):
    ...
    def MapPlot(self, saveFile=None, useText=False):
        ...
        if (
            saveFile is not None and useText
        ):  # Normally skip all the text labels... they are painfully slow!
                ...
                if i > 0:
                    ...
                    if math.isnan(cm[0]) or math.isnan(cm[1]):
                        ...
                        if i >= len(self.mapCentroids):
                            self.mapCentroids += [None] * (
                                1 + i - len(self.mapCentroids)
                            )
                        ...
                        if self.mapCentroids[i] is not None:
                            ...
                            self.mapCentroids[i] = self.ax2.text(
                                y - 5, x + 5, str(i), fontsize=10
                            )
                            # self.mapCentroids[i].set_visible(True)
            ...
            if self.rgbM is None:
                self.rgbM = np.ascontiguousarray(
                    np.array([f1(wi), f2(wi), f3(wi)], dtype=np.uint8).transpose(
                        1, 2, 0
                    )
                )
            else:
            ...
            if self.mapPlot is not None:
                ...
                self.mapPlot = self.ax2.imshow(
                    self.watershed[self.index],
                    animated=True,
                    interpolation="nearest",
                    cmap=self.mapPlotCmap,
    ...
    def ColorPlot(self):
        ...
        if self.rgba is None:
            self.rgba = np.array(
                [
                    ...
                    255 * seed,
                    255 * outl,
                    self.overlayVisible * 255 * seedORoutl,
                ]
            ).transpose(1, 2, 0)
        else:
            self.rgba[:, :, 0] = 0 * seed
        ...
        if self.colorplot is not None and self.bgPlot is not None:
            ...
            self.bgPlot = self.ax1.imshow(
                im, cmap=plt.cm.gray, interpolation="nearest", animated=True
            )
            self.colorplot = self.ax1.imshow(
                self.rgba, interpolation="nearest", animated=True
            )
        ...
        if self.point_mode == False:
            ...
            for i, v in enumerate(self.selectionVals):
                if v > 1:
                    ...
                    cm[i] = center_of_mass(
                        (self.watershed[self.index] == v).astype(np.float)
                    )
    ...
    def HighlightRegion(self):
        ...
        if self.point_mode:
            ...
            for i in self.selectionVals:
                if i > 0:
                    a = a + 255 * (
                        self.watershed[self.index] == i
                    )  # Changed from += due to a regression in numpy
            if self.rgbaH is None:
                self.rgbaH = np.array([a, a, a * 0, a // 2]).transpose(1, 2, 0)
            else:
                self.rgbaH[:, :, 0] = (
                    a // 2 - 1
                )  # These 2 lines used to be just "=a", but then the behavior of
                self.rgbaH[:, :, 1] = (
                    a // 2 - 1
                )  # matplotlib changed and the yellow highlight turned gray
                self.rgbaH[:, :, 2] = a * 0
                self.rgbaH[:, :, 3] = a // 2
            self.DrawBWDot()
        ...
        if self.hiplot is not None:
            ...
            self.hiplot = self.ax1.imshow(
                self.rgbaH, interpolation="nearest", animated=True
            )
    ...
    def RingsPlot(
        self, bins, frame, woundVals, useNei=False, binSize=None, initR=0
    ):  # still using pyplot directly...
        # b,g,r,c,m,y,k
        colors = [
            [0, 0, 255],
            [0, 255, 0],
            ...
            [255, 0, 255],
            [255, 255, 0],
        ] * 10  # that ought to do it ;-P
        rgbMap4Rings = 255 * np.ones(
            list(self.watershed[0].shape) + [3], dtype=np.uint8
        )
        ...
        for i in range(len(bins)):
            for j in range(len(bins[i])):
                ind = i + 2
                val = bins[i][j] + 2
                rgbMap4Rings[np.where(self.watershed[frame] == val)] = colors[
                    ind - 2 + len(woundVals)
                ]
        ...
        if not useNei:
            cir = plt.Circle(
                (self.woundCenters[0][0], self.woundCenters[0][1]),
                radius=0.1,
                Fill=False,
            )
            a.add_patch(cir)
...
class SegmenterFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.MouseModeRadioBox = wx.RadioBox(
            self.MainPanel,
            -1,
            "Mouse Mode",
            choices=[
        ...
        self.ToggleViewCheckbox = wx.CheckBox(
            self.MainPanel, -1, "Show Segmentation Overlays?"
        )
        self.InvertViewCheckbox = wx.CheckBox(
            self.MainPanel, -1, "Show Image Inverted?"
        )
        ...
        self.HighlightListOfCellIDsButton = wx.Button(
            self.MainPanel, -1, "Highlight a List of Cell IDs"
        )
        self.NotesTextBox = wx.TextCtrl(
            self.MainPanel,
            -1,
            "Type Notes Here (per frame)",
            style=wx.TE_PROCESS_ENTER | wx.TE_MULTILINE,
        )
        self.ReSaveAllFramesButton = wx.Button(self.MainPanel, -1, "Re-Save All Frames")
        self.ReRunAllWatershedsButton = wx.Button(
            self.MainPanel, -1, "Re-Run All Watersheds"
        )
        self.CompressSeedValuesButton = wx.Button(
            self.MainPanel, -1, "Compress Seed Values"
        )
        self.AutoCenterWoundButton = wx.Button(self.MainPanel, -1, "Auto Center Wound")
        self.RunCalculationsButton = wx.Button(
            self.MainPanel, -1, "Run Calculations for All Frames"
        )
        self.RunCalculations2Button = wx.Button(
            self.MainPanel, -1, "Run Calculations (2) for All Frames"
        )
        self.CheckForMalformedRegionsButton = wx.Button(
            self.MainPanel, -1, "Check for malformed regions"
        )
        self.StatusText = wx.StaticText(
            self.MainPanel, -1, "Status:\n    Initializing..."
        )
        self.SummaryText = wx.StaticText(
            self.MainPanel,
            -1,
            """
...
""",
        ...
        self.NotesTextBox.SetInitialSize(
            (-1, 100)
        )  # Set the initial height to be 100px

        self.menuBar = wx.MenuBar()
        self.FileMenu = wx.Menu()
        item = self.FileMenu.Append(
            -1, "O&pen Stack or Sequence (Gif/Tiff)\tCtrl-O", "Open Gif/Tiff"
        )
        ...
        for keyCommand in letterKeys.split("\n"):
            ...
            if key in "QO":
                continue
            item = self.CommandMenu.Append(
                -1, "&" + key + " - " + descr + "\tCtrl-" + key, ""
            )
        ...
        for i in [
            ...
            self.MainPanel,
            self.FilenameText,
            self.MouseModeRadioBox,
            self.MouseModeHelpText,
            self.FrameNumber,
            self.FrameNumberText,
            self.ToggleViewCheckbox,
            self.InvertViewCheckbox,
            self.CellNumber,
            self.CellNumberText,
            self.HighlightListOfCellIDsButton,
            self.NotesTextBox,
            self.ReSaveAllFramesButton,
            self.ReRunAllWatershedsButton,
            self.CompressSeedValuesButton,
            self.AutoCenterWoundButton,
            self.RunCalculationsButton,
            self.RunCalculations2Button,
        ...
        self.Bind(
            wx.EVT_RADIOBOX, self.MouseModeRadioBoxCallback, self.MouseModeRadioBox
        )
        self.Bind(wx.EVT_SPINCTRL, self.FrameNumberCallback, self.FrameNumber)
        self.Bind(
            wx.EVT_CHECKBOX, self.ToggleViewCheckBoxCallback, self.ToggleViewCheckbox
        )
        self.Bind(
            wx.EVT_CHECKBOX, self.InvertViewCheckBoxCallback, self.InvertViewCheckbox
        )
        self.Bind(wx.EVT_SPINCTRL, self.CellNumberCallback, self.CellNumber)
        self.Bind(
            wx.EVT_BUTTON,
            self.HighlightListOfCellIDsCallback,
            self.HighlightListOfCellIDsButton,
        )
        # self.Bind(wx.EVT_TEXT, self.NotesTextBoxCallback, self.NotesTextBox)
        self.Bind(
            wx.EVT_BUTTON, self.ReSaveAllFramesCallback, self.ReSaveAllFramesButton
        )
        self.Bind(
            wx.EVT_BUTTON,
            self.ReRunAllWatershedsCallback,
            self.ReRunAllWatershedsButton,
        )
        self.Bind(
            wx.EVT_BUTTON,
            self.CompressSeedValuesCallback,
            self.CompressSeedValuesButton,
        )
        self.Bind(
            wx.EVT_BUTTON, self.AutoCenterWoundCallback, self.AutoCenterWoundButton
        )
        self.Bind(
            wx.EVT_BUTTON, self.RunCalculationsCallback, self.RunCalculationsButton
        )
        self.Bind(
            wx.EVT_BUTTON, self.RunCalculations2Callback, self.RunCalculations2Button
        )
        self.Bind(
            wx.EVT_BUTTON,
            self.CheckForMalformedRegionsCallback,
            self.CheckForMalformedRegionsButton,
        )
    ...
    def OnInit(
        ...
        self.FrameNumberText.SetLabel(
            "Frame Number (last - " + str(self.wd.length - 1) + ")\n(last active - 0)"
        )
        ...
        if not saveDir:
            saveDir = wx.DirSelector(msg, self.saveDir)
    ...
    def SetMPLKeyConnections(self):
        for fig, ax in [
            ...
            ax.format_coord = GetReportPixel(
                self
            )  # Made it so we can also just pass self here...
    ...
    def FileNameTextCallback(self, event):
        ...
        if os.path.exists(f):
            ...
            d = wx.DirSelector(
                "Optionally Pick Directory Where Seed Info is Stored", self.saveDir
            )
            if d != u"":
                self.saveDir = d
                self.wd.Open(d)
    ...
    def UpdateFrameLabelText(self):
        self.FrameNumberText.SetLabel(
            "Frame Number (last - "
            + str(self.wd.length - 1)
            + ")\n(last active - "
            + str(self.wd.GetLastActiveFrame())
            + ")"
        )
    ...
    def FrameNumberCallback(self, event):
        ...
        i2 = max(0, ind - 1)
        if (
            self.wd.sparseList[i2] is not None and ind < self.wd.length
        ):  # If frame before the one to move to is defined
    ...
    def PlotListOfCellIDs(self, selectedCellIDs):
        ...
        self.wd.ax3.format_coord = GetReportPixel(
            self
        )  # Made it so we can also just pass self here...
        self.wd.ax3.imshow(
            sum([(self.wd.watershed[self.wd.index] == v) * v for v in selectedCellIDs]),
            interpolation="nearest",
            cmap=self.wd.mapPlotCmap,
            norm=matplotlib.colors.NoNorm(),
        )
    ...
    def HighlightListOfCellIDsCallback(self, event):
        ...
        dlg = wx.TextEntryDialog(
            None, "Which Cell IDs do you want to plot?", "Cell IDs", "#,#"
        )
    ...
    def ReRunAllWatershedsCallback(self, event):
        ...
        oldAlgo = self.wd.walgorithm[
            self.wd.index
        ]  # make sure to put things back like we found them...
        self.wd.UpdateSeeds()
        ...
        self.wd.walgorithm[
            self.wd.index
        ] = oldAlgo  # set algorithm back (in case it was different from PyMorph...)
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            for i in range(numFrames):
                self.SetStatus(
                    "Running Watershed on Frame " + str(i) + " out of " + str(numFrames)
                )
                self.wd.index = i
                self.wd.UpdateSeeds()
    ...
    def CompressSeedValuesCallback(self, event):
        ...
        dlg = wx.MessageDialog(
            self,
            "Compressing Seed Values removes unused seed values,\n"
    ...
    def AutoCenterWoundCallback(self, event):
        ...
        dlg = wx.TextEntryDialog(
            None,
            "Which cells are wound (enter a comma-separated list of cell values)",
            "Wound Values",
            "#,#",
        )
        if dlg.ShowModal() == wx.ID_OK:
    ...
    def RunCalculationsCallback(self, event):
        ...
        for index in range(self.wd.length):
            ...
            if self.wd.woundCenters[index] is None:
                s = (
                    "You must use Center Mode to define a"
                    + os.linesep
                    + "center for all initialized frames (failed on frame"
                    + str(index)
                    + ")!"
                )
                wx.MessageBox(s)
    ...
    def CheckForMalformedRegionsCallback(self, event):
        ...
        wx.TextCtrl(
            dia, -1, s, style=wx.TE_MULTILINE | wx.TE_READONLY
        ...
        if s == "No malformed regions found!":
            statusStr = (
                "Attempting to make static and matched CellNetwork json files..."
            )
            print(statusStr)
            self.SetStatus(statusStr)

            allMatched = SWHelpers.MakeCellNetworkJsonFiles(
                self.wd.watershed, self.saveDir
            )
            wx.MessageBox(
                "Made matched and non-matched json files!"
                if allMatched
                else "Matching failed on some frames! Made json files anyway."
            )
    ...
    def HandleMPLMouseEvents(self, event):
        if None not in [event.xdata, event.ydata]:
            if self.mouseMode == "m":
                ...
                if self.wd.fig1.canvas.widgetlock.locked():
                    return
                ...
                if self.wd.watershed is not None and self.wd.woutline is not None:
                    if event.inaxes:
                        if event.button == 1:
                            ...
                            print(
                                "Switch all points with values:",
                                self.wd.selectionVals[0],
                                "and",
                                self.wd.watershed[self.wd.index][y, x],
                            )
                            self.wd.SwitchRegionValues(
                                self.wd.selectionVals[0],
                                self.wd.watershed[self.wd.index][y, x],
                            )
                            self.wd.ColorPlot()
                ...
                if self.wd.watershed is not None and self.wd.woutline is not None:
                    if event.inaxes:
                        if event.button == 1:
                            ...
                            print(
                                " 2: switch into a new sub-mode that lets me draw 2 lines"
                            )
                        ...
                        #
                            ...
                            if self.wd.previousDrawPoint is None:
                                ...
                                self.wd.ExtraSeedLine(
                                    [y, x], [y, x], self.wd.selectionVals[0]
                                )
                            else:  # draw a line on the image
                                self.wd.ExtraSeedLine(
                                    self.wd.previousDrawPoint,
                                    [y, x],
                                    self.wd.selectionVals[0],
                                )
    ...
    def HandleMPLandWxKeyEvents(self, ckey):
        if ckey == "o":  # Open new file
            ...
            f = wx.FileSelector(
                "Select a tiff or gif file", default_filename=self.filename
            )
            if os.path.exists(f):
            ...
            dlg = wx.TextEntryDialog(None, "Sigma Value?", "Gauss Average Data", "4.0")
            if dlg.ShowModal() == wx.ID_OK:
                ...
                if sigma != None:
                    ...
                    if self.wd.sparseList[self.wd.index] is None:
                        ...
                        dlg = wx.MessageDialog(
                            self,
                            "Are you sure you want revert any manual seed changes?",
                ...
                if self.wd.sparseList[self.wd.index] is None:
                    sh = self.wd.shape[1:]
                    self.wd.sparseList[self.wd.index] = scipy.sparse.lil_matrix(
                        sh, dtype=np.uint16
                    )
                    # I guess I could change the dtype later if I need to...
                    self.wd.seedSelections[self.wd.index] = scipy.sparse.lil_matrix(
                        sh, dtype=np.bool
                    )
            ...
            if not STEAL_B_KEY:
                ...
                dlg = wx.TextEntryDialog(
                    None, "Sigma Value?", "Background Subtraction", "64.0"
                )
                if dlg.ShowModal() == wx.ID_OK:
            ...
            dlg = wx.TextEntryDialog(
                None,
                "Two Comma-separated values for Sharpen Filter?",
                "Sharpen Data",
                "2.0,6.0",
            )
            if dlg.ShowModal() == wx.ID_OK:
            ...
            dlg = wx.TextEntryDialog(
                None, "Median Filter Size?", "Median Filter Data", "3"
            )
            if dlg.ShowModal() == wx.ID_OK:
        ...
        #        print('Use PyMorph instead!')
            ...
            np.clip(
                wh[0], 0, self.wd.shape[1]
            )  # This is an ok solution, but there is probably a better one...
            ...
            self.wd.sparseList[self.wd.index] = scipy.sparse.lil_matrix(
                self.wd.seedArray, dtype=np.uint16
            )
            ...
            if self.shiftDown and USE_DEBUG_PRINT:  # Aka, if it's a developer...
                te = wx.TextEntryDialog(
                    None,
                    "Inject Code... very dangerous, but fun!",
            ...
            if not self.wd.point_mode:
                if self.wd.watershed is not None and self.wd.woutline is not None:
                    ...
                    dlg = wx.TextEntryDialog(
                        None, "New Value?", "Pick a new value for the region.", ""
                    )
                    if dlg.ShowModal() == wx.ID_OK:
                        newVal = int(dlg.GetValue())
                        self.wd.ChangeRegionValue(
                            self.wd.selectionVals[0], newVal
                        )  # NEEDS UNDO
                        self.wd.ColorPlot()
                        self.wd.MapPlot()
        elif ckey == "q":  # Quit
...
if __name__ == "__main__":
    ...
    f = (
        wx.FileSelector("Select a tiff or gif file")
        if len(sys.argv) <= 1
